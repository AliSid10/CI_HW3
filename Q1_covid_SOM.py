...
class SOM:
            ...
            for row in reader:
                if c != 0:
                    ...
                    if row[1] not in list(self.data.keys()):
                        self.data[row[1]] = [int(row[5]),int(row[6]),int(row[7])]
                    else:
                        temp = [self.data[row[1]][0] + int(row[5]), self.data[row[1]][1] + int(row[6]), self.data[row[1]][2] + int(row[7])]
    ...
    def neighborhood(self,target, current, epoch):
        if self.decay_L == 1:
            return (1/(math.sqrt((target[1]-current[1])**2 + (target[0]-current[0])**2)))*(math.exp(-epoch/self.epoch))
        else:
            return (1/(math.sqrt((target[1]-current[1])**2 + (target[0]-current[0])**2)))*((self.epoch-epoch)/self.epoch)
        ...
        for i in range(self.n):
            for j in range(self.n):
                ...
                if neuron[0] == i and neuron[1] == j:
                    self.weights[i][j][0] = self.weights[i][j][0] + (self.L(epoch)*(inp[0] - self.weights[i][j][0]))
                    self.weights[i][j][1] = self.weights[i][j][1] + (self.L(epoch)*(inp[1] - self.weights[i][j][1]))
                    self.weights[i][j][2] = self.weights[i][j][2] + (self.L(epoch)*(inp[2] - self.weights[i][j][2]))
                else:
                    self.weights[i][j][0] = self.weights[i][j][0] + (self.L(epoch)*self.neighborhood(neuron,[i,j],epoch)*(inp[0] - self.weights[i][j][0]))
                    self.weights[i][j][1] = self.weights[i][j][1] + (self.L(epoch)*self.neighborhood(neuron,[i,j],epoch)*(inp[1] - self.weights[i][j][1]))
                    self.weights[i][j][2] = self.weights[i][j][2] + (self.L(epoch)*self.neighborhood(neuron,[i,j],epoch)*(inp[2] - self.weights[i][j][2]))
    ...
    def get_BMU(self, inp):
        ...
        for i in range(self.n):
            for j in range(self.n):
                d = math.sqrt((self.weights[i][j][0] - inp[0])**2 + (self.weights[i][j][1] - inp[1])**2 + (self.weights[i][j][2] - inp[2])**2)
                if d <= best:
                    best = d
                    BMU = [i,j]
        return BMU
...
som = SOM(n = 10, alpha = 0.5, epoch = 10000, decay_L = 1, decay_N = 1) 
   
# som.readData('Q1_countrydata.csv')
